@php
    ...
    $destination = trim(($rental->municipality ?? '') . ( $rental->province ? (', ' . $rental->province) : ''));
@endphp
...
<div style="margin:0; padding:0; width:100%; background:#f1f5f9;">
    <div style="max-width: 720px; margin: 0 auto; padding: 24px 14px;">
        <div style="background:#0f172a; border-radius: 14px; padding: 18px 18px; color:#f8fafc;">
            ...
            <div style="margin-top: 6px; color:#cbd5e1; font-weight: 700;">Booking confirmed</div>
        </div>
        ...
        <div style="margin-top: 14px; background:white; border: 1px solid #e2e8f0; border-radius: 14px; overflow:hidden;">
            ...
            <div style="padding: 16px 18px;">
                <div style="background:#f0fdf4; border: 1px solid #dcfce7; border-radius: 12px; padding: 14px;">
                    <div style="font-weight: 900; color:#16a34a;">Your booking is confirmed</div>
                    <div style="margin-top: 8px; color:#166534; font-weight: 700;">Thank you for choosing Auto Amegos Rent-a-Car.</div>
                </div>
                ...
                <div style="margin-top: 12px; display:grid; grid-template-columns: 1fr; gap: 12px;">
                    ...
                    <div style="background:#f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 14px;">
                        <div style="font-weight: 900; color:#0f172a;">Pickup & Destination</div>
                        <div style="margin-top: 8px; color:#0f172a; font-weight: 700;">Pickup Location: {{ $rental->pickup_location ?? '—' }}</div>
                        <div style="margin-top: 4px; color:#0f172a; font-weight: 700;">Destination: {{ $destination ?: '—' }}</div>
                    </div>

                    <div style="background:#f8fafc; border: 1px solid #e2e8f0; border-radius: 12px; padding: 14px;">
                        <div style="font-weight: 900; color:#0f172a;">Cost Breakdown (Estimate)</div>
                        <div style="margin-top: 8px; color:#0f172a; font-weight: 700;">Destination Charge: ₱{{ number_format((float) ($rental->destination_price ?? 0), 2) }}</div>
                        <div style="margin-top: 4px; color:#0f172a; font-weight: 700;">Carwash Fee: ₱{{ number_format((float) ($rental->carwash_fee ?? 0), 2) }}</div>
                        <div style="margin-top: 4px; color:#0f172a; font-weight: 700;">Extra Hours Fee: ₱{{ number_format((float) ($rental->extra_hours_fee ?? 0), 2) }}</div>
                        <div style="margin-top: 8px; color:#0f172a; font-weight: 900;">Estimated Total: ₱{{ $amount }}</div>
                    </div>

                    <div style="background:#ffffff; border: 1px dashed rgba(245,158,11,0.55); border-radius: 12px; padding: 14px;">
                        <div style="font-weight: 900; color:#0f172a;">Next Steps</div>
                        <ul style="margin-top: 10px; padding-left: 18px; color:#0f172a; font-weight: 700; line-height: 1.5;">
                            <li>Arrive on time at the pickup location with a valid government-issued ID.</li>
                            <li>Bring your driver’s license (must be valid and readable).</li>
                            <li>Prepare any payment proof or screenshots if required by the owner.</li>
                        </ul>
                    </div>
                </div>
