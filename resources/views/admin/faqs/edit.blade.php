<x-member-layout>
    <x-slot name="header">
        <div style="display:flex; align-items:flex-start; justify-content:space-between; gap: 12px; flex-wrap: wrap;">
            <div>
                <h2>{{ __('Edit FAQ') }}</h2>
                <p style="color: #64748b; margin-top: 5px;">Update an FAQ entry.</p>
            </div>
    ...
    <div class="container" style="padding: 0 0 40px 0; margin-left: 20px; margin-right: 20px; width: calc(100% - 40px);">
        <div class="admin-layout">
            ...
            <div class="admin-content" style="background: white; border: 1px solid #e2e8f0; border-radius: 12px; padding: 18px; box-shadow: var(--shadow-sm);">
                <form method="POST" action="{{ route('admin.faqs.update', $faq) }}" style="display:grid; gap: 14px;">
                    @csrf
                    @method('PUT')

                    <div>
                        <label style="display:block; font-weight: 900; color:#0f172a; margin-bottom: 6px;">Question</label>
                        <input type="text" name="question" value="{{ old('question', $faq->question) }}" style="width:100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 10px;">
                    ...
                    <div>
                        ...
                        <textarea name="answer" rows="10" style="width:100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 10px; resize: vertical;">{{ old('answer', $faq->answer) }}</textarea>
                    ...
                    <div style="display:flex; gap: 12px; flex-wrap: wrap; align-items:center;">
                        <div style="min-width: 180px;">
                            ...
                            <input type="number" name="sort_order" value="{{ old('sort_order', (int) $faq->sort_order) }}" style="width:100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 10px;">
                        ...
                        <label style="display:flex; gap: 10px; align-items:center; cursor:pointer; margin-top: 28px;">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $faq->is_active ? '1' : '') ? 'checked' : '' }} style="width: 18px; height: 18px; accent-color: var(--accent);">
                    ...
                    <div style="display:flex; gap: 10px; justify-content:flex-end; flex-wrap:wrap;">
                        ...
                        <button type="submit" class="btn btn-primary" style="padding: 10px 14px;">Save Changes</button>
                    </div>
                </form>
            </div>
        </div>
